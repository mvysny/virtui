...
describe Virt::BallooningVM::MemLevelShrinkVoter do
  ...
  it 'votes at exactly the trigger' do
    v = voter
    v.observe sample(55)
    assert_equal 'usage is at or under the 55% trigger', v.vote_reason
  end

  it 'stays silent one point above it' do
    v = voter
    v.observe sample(56)
    assert_nil v.vote_reason
  end

  it 'votes for a guest that is nearly empty' do
    v = voter
    v.observe sample(1)

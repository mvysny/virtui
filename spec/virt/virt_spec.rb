...
describe VirtCmd do
  ...
  it 'domain_data' do
    result = VirtCmd.new.domain_data(File.read('spec/virt/domstats0.txt'), 0)
  ...
  it 'cpu usage' do
    ...
    result0 = VirtCmd.new.domain_data(File.read('spec/virt/domstats0.txt'), millis_since_epoch)['ubuntu']
    result1 = VirtCmd.new.domain_data(File.read('spec/virt/domstats1.txt'), millis_since_epoch + 10 * 1000)['ubuntu']
    assert_equal 22.51, result1.cpu_usage(result0).round(2)
    result2 = VirtCmd.new.domain_data(File.read('spec/virt/domstats2.txt'), millis_since_epoch + 20 * 1000)['ubuntu']

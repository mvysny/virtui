...
module Virt
  ...
  # failing is written off for a while (see {FAILURES_BEFORE_BACKOFF}).
  #
  ...
  class GuestAgent
    ...
    # How many consecutive failures write a guest off, and for how long. A guest with no
    # agent fails every poll, at one log line per VM per 2s tick and up to
    # {TIMEOUT_SECONDS} of the timer thread each time; three strikes still tells a blip (a
    # guest mid-boot) from a guest that cannot answer, and five minutes heals — an agent
    # installed, a guest rebooted — without restarting virtui.
    ...
    #   (see {BACKOFF_SECONDS}); lowered by the specs, which would otherwise have to wait
    #   five minutes to watch a write-off lapse
    ...
    #   cleared here, so the next call starts from a clean slate
    ...
    private def note_failure(domain, error)
      ...
      if count < FAILURES_BEFORE_BACKOFF
        ...
        $log.info("#{domain}: guest agent gives no swap level (#{reason}); not asking again " \
